...
int set_nvidia_func_ptrs(int idx)
{
    ...
    if (*g_platform[idx].arch_id == VOLTA)
    {
        ...
        g_platform[idx].variorum_get_gpu_utilization_json = volta_get_gpu_utilization_json;
        /* Initialize control interfaces */
        g_platform[idx].variorum_cap_each_gpu_power_limit = volta_cap_each_gpu_power_limit;
