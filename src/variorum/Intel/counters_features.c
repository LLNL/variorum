...
void set_fixed_counter_ctrl(struct fixed_counter *ctr0,
                            struct fixed_counter *ctr1, struct fixed_counter *ctr2, off_t msr1, off_t msr2)
{
    ...
    for (i = 0; i < nthreads; i++)
    {
        ...
        *ctr1->value[i] = 0;
        *ctr2->value[i] = 0;
        *perf_global_ctrl[i] = (*perf_global_ctrl[i] & ~(1ULL << 32)) | ctr0->enable[i]
                               << 32;
        *perf_global_ctrl[i] = (*perf_global_ctrl[i] & ~(1ULL << 33)) | ctr1->enable[i]
                               << 33;
        *perf_global_ctrl[i] = (*perf_global_ctrl[i] & ~(1ULL << 34)) | ctr2->enable[i]
                               << 34;

        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(3ULL << 0))) |
                             (ctr0->ring_level[i] << 0);
        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(1ULL << 2))) |
                             (ctr0->anyThread[i] << 2);
        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(1ULL << 3))) |
                             (ctr0->pmi[i] << 3);

        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(3ULL << 4))) |
                             (ctr1->ring_level[i] << 4);
        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(1ULL << 6))) |
                             (ctr1->anyThread[i] << 6);
        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(1ULL << 7))) |
                             (ctr1->pmi[i] << 7);

        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(3ULL << 8)))  |
                             (ctr2->ring_level[i] << 8);
        *fixed_ctr_ctrl[i] = (*fixed_ctr_ctrl[i] & (~(1ULL << 10))) |
                             (ctr2->anyThread[i] << 10);
        ...
        switch (avail)
        {
            case 8:
                cfprintf(writedest, "%s %s %d %lu %lu %lu %lu %lu %lu %lu %lu\n",
                         ...
                         *p->pmc2[i], *p->pmc3[i], *p->pmc4[i], *p->pmc5[i], *p->pmc6[i], *p->pmc7[i]);
                break;
            case 7:
...
void print_verbose_fixed_counter_data(FILE *writedest, off_t *msrs_fixed_ctrs)
{
    ...
    {
        fprintf(writedest,
                "_FIXED_COUNTERS Host: %s, Thread: %d, InstRet: %lu, UnhaltClkCycles: %lu, UnhaltRefCycles: %lu\n",
                                        ...
                                        off_t *msrs_perfevtsel_ctrs, off_t *msrs_perfmon_ctrs)
{
    ...
    for (i = 0; i < nthreads; i++)
    {
        switch (avail)
        {
            case 8:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu, PMC2: %lu, PMC3: %lu, PMC4: %lu, PMC5: %lu, PMC6: %lu, PMC7: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i], *p->pmc2[i], *p->pmc3[i], *p->pmc4[i],
                        *p->pmc5[i], *p->pmc6[i], *p->pmc7[i]);
                break;
            case 7:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu, PMC2: %lu, PMC3: %lu, PMC4: %lu, PMC5: %lu, PMC6: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i], *p->pmc2[i], *p->pmc3[i], *p->pmc4[i],
                        *p->pmc5[i], *p->pmc6[i]);
                break;
            case 6:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu, PMC2: %lu, PMC3: %lu, PMC4: %lu, PMC5: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i], *p->pmc2[i], *p->pmc3[i], *p->pmc4[i],
                        *p->pmc5[i]);
                break;
            case 5:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu, PMC2: %lu, PMC3: %lu, PMC4: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i], *p->pmc2[i], *p->pmc3[i], *p->pmc4[i]);
                break;
            case 4:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu, PMC2: %lu, PMC3: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i], *p->pmc2[i], *p->pmc3[i]);
                break;
            case 3:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu, PMC2: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i], *p->pmc2[i]);
                break;
            case 2:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu, PMC1: %lu\n",
                        hostname, i, *p->pmc0[i], *p->pmc1[i]);
                break;
            case 1:
                fprintf(writedest,
                        "_PERFORMANCE_MONITORING_COUNTERS Host: %s, Thread: %d, PMC0: %lu\n",
                        hostname, i, *p->pmc0[i]);
                break;
        }
    }
...
void set_pmc_ctrl_flags(uint64_t cmask, uint64_t flags, uint64_t umask,
                        uint64_t eventsel, int pmcnum, int thread, off_t *msrs_perfevtsel_ctrs)
{
    static struct perfevtsel *evt = NULL;
    if (evt == NULL)
    {
    ...
    {
        case 8:
            *evt->perf_evtsel7[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 7:
            *evt->perf_evtsel6[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 6:
            *evt->perf_evtsel5[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 5:
            *evt->perf_evtsel4[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 4:
            *evt->perf_evtsel3[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 3:
            *evt->perf_evtsel2[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 2:
            *evt->perf_evtsel1[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
        case 1:
            *evt->perf_evtsel0[thread] = 0UL | (cmask << 24) | (flags << 16) |
                                         (umask << 8) | eventsel;
            break;
    }
}
                                    ...
                                    off_t *msrs_pcu_pmon_evtsel, off_t *msrs_pcu_pmon_ctrs)
{
    ...
    {
        fprintf(writedest,
                "_UNCORE_COUNTERS Host: %s, Socket: %d, c0: 0x%lx, c1: 0x%lx, c2: 0x%lx, c3: 0x%lx\n",
                hostname, i, *uc->c0[i], *uc->c1[i], *uc->c2[i], *uc->c3[i]);
    }
}

void get_all_power_data_fixed(FILE *writedest, off_t msr_pkg_power_limit,
                              off_t msr_dram_power_limit, off_t msr_rapl_unit,
                              off_t msr_package_energy_status, off_t msr_dram_energy_status,
                              off_t *msrs_fixed_ctrs, off_t msr_perf_global_ctrl,
                              off_t msr_fixed_counter_ctrl, off_t msr_aperf, off_t msr_mperf, off_t msr_tsc)
{
    ...
    if (!init_get_power_data)
    {
        ...
        for (i = 0; i < nthreads; i++)
        {
            snprintf(thread_strs[i][0], max_str_len, "InstRet%d", i);
            snprintf(thread_strs[i][1], max_str_len, "UnhaltClkCycles%d", i);
            snprintf(thread_strs[i][2], max_str_len, "UnhaltRefCycles%d", i);
            snprintf(thread_strs[i][3], max_str_len, "APERF%d", i);
            snprintf(thread_strs[i][4], max_str_len, "MPERF%d", i);
