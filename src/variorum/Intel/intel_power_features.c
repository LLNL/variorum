...
static int translate(const int socket, uint64_t *bits, double *units,
...
static int calc_rapl_bits(const int socket, struct rapl_limit *limit,
                          const int offset, off_t msr)
{
...
static int calc_rapl_from_bits(const int socket, struct rapl_limit *limit,
                               const int offset, off_t msr)
{
    uint64_t watts_bits = 0;
    uint64_t seconds_bits = 0;
...
static int calc_package_rapl_limit(const int socket,
...
static int calc_dram_rapl_limit(const int socket, struct rapl_limit *limit,
                                off_t msr)
{
    if (limit != NULL)
    {
...
int get_package_rapl_limit(const int socket, struct rapl_limit *limit1,
...
int get_dram_rapl_limit(const int socket, struct rapl_limit *limit,
...
int cap_package_power_limit(const int socket, int package_power_limit,
                            off_t msr_power_limit, off_t msr_rapl_unit)
{
    uint64_t val = 0;
