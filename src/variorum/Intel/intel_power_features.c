...
#ifdef CPRINTF_FOUND
        cfprintf(writedest, "%s %#lx %s %d %#lx %lf %lf %lf %lf\n",
                "_PACKAGE_POWER_LIMIT", msr_power_limit, hostname, socket, l1.bits, l1.watts, l1.seconds, l2.watts,
                l2.seconds);
#else
        fprintf(writedest, "%s %#lx %s %d %#lx %lf %lf %lf %lf\n",
                "_PACKAGE_POWER_LIMIT", msr_power_limit, hostname, socket, l1.bits, l1.watts, l1.seconds, l2.watts,
                            ...
                            off_t msr_rapl_unit, int socket)
{
    ...
    if (!get_dram_rapl_limit(socket, &l1, msr_power_limit, msr_rapl_unit))
    {
        ...
        cfprintf(writedest, "%s %#lx %s %d %#lx %lf %lf\n",
                "_DRAM_POWER_LIMIT", msr_power_limit, hostname, socket, l1.bits, l1.watts, l1.seconds);
#else
...
#ifdef CPRINTF_FOUND
        cfprintf(writedest, "%s %#lx %s %d %#lx %lf %lf %lf %lf\n",
                "_DRAM_ENERGY_STATUS", msr_dram_energy_status, hostname, i, *rapl->dram_bits[i], rapl->dram_joules[i],
                rapl->dram_watts[i], rapl->elapsed,
                now.tv_sec - start.tv_sec + (now.tv_usec - start.tv_usec) / 1000000.0);
#else
        fprintf(writedest, "%s %#lx %s %d %#lx %lf %lf %lf %lf\n",
                        ...
                        off_t msr_package_energy_status, off_t msr_dram_energy_status)
{
    ...
    {
#ifdef CPRINTF_FOUND
        cfprintf(writedest, "%s %ld %lf %lf %lf %lf %lf ", "_POWMON", now_ms(), rapl->pkg_delta_joules[i],
                rlim[rlim_idx].watts, rlim[rlim_idx + 1].watts, rapl->dram_delta_joules[i],
                rlim[rlim_idx + 2].watts);
#else
        fprintf(writedest, " %s %ld %lf %lf %lf %lf %lf", "_POWMON", now_ms(), rapl->pkg_delta_joules[i],
                rlim[rlim_idx].watts, rlim[rlim_idx + 1].watts, rapl->dram_delta_joules[i],
                rlim[rlim_idx + 2].watts);
#endif
        rlim_idx += 3;
