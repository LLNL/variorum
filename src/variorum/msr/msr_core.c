...
static uint64_t devidx(unsigned socket, unsigned core, unsigned thread)
{
    ...
    variorum_get_topology(&nsockets, &ncores, &nthreads, MSR_CORE_IDX);
...
static int *core_fd(const unsigned dev_idx)
{
    ...
    {
        init_core_fd = 1;
        variorum_get_topology(NULL, NULL, &nthreads, MSR_CORE_IDX);
...
int sockets_assert(const unsigned *socket)
{
    ...
    unsigned nsockets;
    variorum_get_topology(&nsockets, NULL, NULL, MSR_CORE_IDX);
...
int cores_assert(const unsigned *core)
{
    char *variorum_error_msg = malloc(NAME_MAX * sizeof(char));
    unsigned ncores;
    variorum_get_topology(NULL, &ncores, NULL, MSR_CORE_IDX);
