                         ...
                         int **opssize)
{
    ...
    if (batch == NULL)
    {
        ...
        size = (int *) calloc(arrsize, sizeof(int));
        for (i = 0; i < arrsize; i++)
        {
            size[i] = 0;
            batch[i].ops = NULL;
    ...
    else if (batchnum + 1 > (int)arrsize)
    {
        ...
        size = (int *) realloc(size, arrsize * sizeof(int));
...
int sockets_assert(const int *socket)
{
...
int threads_assert(const int *thread)
{
    char *variorum_error_msg = malloc(NAME_MAX * sizeof(char));
    int nthreads;
#ifdef VARIORUM_WITH_AMD_CPU
...
int cores_assert(const int *core)
{
    char *variorum_error_msg = malloc(NAME_MAX * sizeof(char));
    int ncores;
#ifdef VARIORUM_WITH_AMD_CPU
...
int write_msr_by_coord(int socket, int core, int thread,
...
int read_msr_by_coord(int socket, int core, int thread,
                      off_t msr, uint64_t *val)
{
